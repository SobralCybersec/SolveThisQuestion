...
async fn start_login_bridge(
    state: &Arc<AppState>,
) -> Result<super::super::bridge::BridgeProcess, (StatusCode, Json<serde_json::Value>)> {
    let process_lock = super::super::bridge::acquire_bridge_process_lock(&state.runtime)
        .await
        .map_err(|error| {
            reset_login(state);
            (
    ...
    let mut child = super::super::bridge::bridge_command(state, &config)
    ...
    Ok(super::super::bridge::BridgeProcess {
        child,
        stdin,
...
async fn stop_failed_login(
    state: &Arc<AppState>,
    bridge: &mut super::super::bridge::BridgeProcess,
    error: String,
